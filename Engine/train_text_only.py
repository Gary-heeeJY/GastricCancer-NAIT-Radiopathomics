...
from Models.text_net import TextOnlyNet

EVALUATION_STRATEGY = 2


def evaluate_txt(model, dataloader, criterion, device):
    model.eval()
    total_loss, all_labels, all_preds, all_probs = 0.0, [], [], []
    with torch.no_grad():
        for batch in dataloader:
            txt_feat, label = batch['text_feature'].to(device), batch['label'].to(device)
            logits, _ = model(txt_feat)
...
def main(cfg: DictConfig):
    ...
    for fold_idx in range(1, 6):
        ...
        model = TextOnlyNet(cfg.model).to(device)
        ...
        for epoch in range(1, cfg.training.epochs + 1):
            ...
            for batch_idx, batch in enumerate(train_loader):
                logits, _ = model(batch['text_feature'].to(device))
            ...
            train_metrics, val_metrics = evaluate_txt(model, train_eval_loader, criterion, device), evaluate_txt(model,
                                                                                                                 val_loader,
                                                                                                                 criterion,
                                                                                                                 device)
            scheduler.step()
        ...
        final_m = evaluate_txt(model, val_loader, criterion, device)
    ...
    print("\n" + "=" * 60 + "\n🏆 启动 OOF 盲测汇总 (Text 纯文本)\n" + "=" * 60)
    ensemble_models = [TextOnlyNet(cfg.model).to(device) for _ in range(5)]
    ...
    loader_all = DataLoader(
        GastricCancerMultiModalDataset(cfg.paths.original_excel_dir, cfg.paths.feature_dir, mode='all'), batch_size=1,
        shuffle=False, collate_fn=collate_fn_batch_1)
    all_preds = []
    with torch.no_grad():
        for batch in loader_all:
            val_fold_idx = patient_to_val_fold.get(clean_id(batch['patient_id']))
            if val_fold_idx is None: continue
            logits, _ = ensemble_models[val_fold_idx - 1](batch['text_feature'].to(device))
            prob = torch.sigmoid(logits).item()
