...
from Models.wsi_net import WSIOnlyNet

EVALUATION_STRATEGY = 2


def evaluate_wsi(model, dataloader, criterion, device):
    model.eval()
    total_loss = 0.0
    all_labels, all_preds, all_probs = [], [], []
    with torch.no_grad():
        for batch in dataloader:
            img_feat, label = batch['image_features'].to(device), batch['label'].to(device)
            logits, _ = model(img_feat)
...
def main(cfg: DictConfig):
    ...
    for fold_idx in range(1, 6):
        ...
        model = WSIOnlyNet(cfg.model).to(device)
        ...
        for epoch in range(1, cfg.training.epochs + 1):
            ...
            for batch_idx, batch in enumerate(train_loader):
                logits, _ = model(batch['image_features'].to(device))
            ...
            train_metrics, val_metrics = evaluate_wsi(model, train_eval_loader, criterion, device), evaluate_wsi(model,
                                                                                                                 val_loader,
                                                                                                                 criterion,
                                                                                                                 device)
            scheduler.step()
        ...
        final_m = evaluate_wsi(model, val_loader, criterion, device)
    ...
    print("\n" + "=" * 60 + "\n🏆 启动 OOF 盲测汇总 (WSI 纯病理)\n" + "=" * 60)
    ensemble_models = [WSIOnlyNet(cfg.model).to(device) for _ in range(5)]
