...
from Models.text_ct_net import TextCTFusionNet

EVALUATION_STRATEGY = 2


def evaluate_textct(model, dataloader, criterion, device):
    model.eval()
    total_loss, all_labels, all_preds, all_probs = 0.0, [], [], []
    with torch.no_grad():
        for batch in dataloader:
            txt_feat, ct_feat, label = batch['text_feature'].to(device), batch['ct_features'].to(device), batch[
                'label'].to(device)
            logits, _ = model(txt_feat, ct_feat)
...
def main(cfg: DictConfig):
    ...
    for fold_idx in range(1, 6):
        ...
        model = TextCTFusionNet(cfg.model).to(device)
        ...
        for epoch in range(1, cfg.training.epochs + 1):
            ...
            for batch_idx, batch in enumerate(train_loader):
                logits, _ = model(batch['text_feature'].to(device), batch['ct_features'].to(device))
            ...
            train_metrics, val_metrics = evaluate_textct(model, train_eval_loader, criterion, device), evaluate_textct(
        ...
        final_m = evaluate_textct(model, val_loader, criterion, device)
    ...
    print("\n" + "=" * 60 + "\n🏆 启动 OOF 盲测汇总 (Text + CT 双模态)\n" + "=" * 60)
    ensemble_models = [TextCTFusionNet(cfg.model).to(device) for _ in range(5)]
